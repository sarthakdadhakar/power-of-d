...
results="/tmp/results"
recordcount="$1"
exp_results_dir="$home_dir/scan-large-subrange-ten-servers-backup-$recordcount"
dryrun="$2"
...
function run_bench() {
	...
	result_dir_name="nova-s-$nservers-d-$dist-w-$workload-nm-$num_memtables-np-$num_memtable_partitions-s-$enable_subrange-mc-$major_compaction_type-mp-$major_compaction_max_parallism-ms-$major_compaction_max_tables_in_a_set"
    ...
    for s in ${servers[@]}
    do
    	snapshotname="/db/snapshot-$nservers-$number_of_ltcs-$dist-$num_memtable_partitions-$memtable_size_mb-$zipfianconstant-$recordcount"
