...
results="/tmp/results"
recordcount="$1"
exp_results_dir="$home_dir/new-nova-lsm-sr-skewed-impact-$recordcount"
dryrun="$2"
...
function run_bench() {
	...
	result_dir_name="nova-d-$dist-w-$workload-ltc-$number_of_ltcs-stoc-$nstoc-l0-$l0_stop_write_mb-s-$enable_subrange-sn-$subrange_no_flush_num_keys-zf-$zipfianconstant"
...
do
for workload in "workloade" #"workloadw"
do
...
python /proj/bg-PG0/kanakia/scripts/parse_ycsb_nova_leveldb.py $nmachines $exp_results_dir > stats_skewed_impact_out
