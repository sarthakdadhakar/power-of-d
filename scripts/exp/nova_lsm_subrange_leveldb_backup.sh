...
results="/tmp/results"
recordcount="$1"
exp_results_dir="$home_dir/scan-subrange-single-backup-$recordcount"
dryrun="$2"
...
function run_bench() {
    ...
    for s in ${servers[@]}
    do
    	snapshotname="/db/snapshot-$nservers-$number_of_ltcs-$dist-$num_memtable_partitions-$memtable_size_mb-$zipfianconstant"
