...
recordcount="$1"
dryrun="$2"
exp_results_dir="$home_dir/large-wr-nova-lsm-sr-leveldb-10-servers-ranges-$recordcount"
...
function run_bench() {
	...
	result_dir_name="nova-d-$dist-w-$workload-ltc-$number_of_ltcs-stoc-$nstoc-l0-$l0_stop_write_mb-np-$num_memtable_partitions-mp-$major_compaction_max_parallism-log-$log_record_mode-p-$ltc_num_stocs_scatter_data_blocks"
...
do
for workload in "workloada60" "workloada70" "workloada80" "workloada90" 
do
...
python /proj/bg-PG0/kanakia/scripts/parse_ycsb_nova_leveldb.py 25 $exp_results_dir > stats_wr_lsm_leveldb_10_servers_ranges_out_$recordcount
