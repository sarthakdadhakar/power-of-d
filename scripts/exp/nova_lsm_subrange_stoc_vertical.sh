...
results="/tmp/results"
recordcount="$1"
exp_results_dir="$home_dir/new-nova-lsm-sr-vertical-scale-no-compact-$recordcount"
dryrun="$2"
...
function run_bench() {
	...
	result_dir_name="nova-d-$dist-w-$workload-ltc-$number_of_ltcs-stoc-$nstoc-l0-$l0_stop_write_mb-np-$num_memtable_partitions"
...
for num_memtable_partitions in "1" "2" "4" "8" "16" "32" "64"
do
major_compaction_max_parallism=$num_memtable_partitions
num_memtables=$((num_memtable_partitions*2))
run_bench
done


python /proj/bg-PG0/kanakia/scripts/parse_ycsb_nova_leveldb.py $nmachines $exp_results_dir > vertical_stats_out
